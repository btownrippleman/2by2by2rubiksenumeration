...
def generate_for_one_set_of_configs(configs):
	# new_configs = [[[0]*3]*8]*int(1.5*10**6)
	it = 0
	it_valid = 0
	for it in range(configs["numberOfconfigs"]) :
		config = configs["latest_configs"][it]
		for direction in [-1,1]:
			for axis in "xyz":
				c = rotate(config,direction,axis)
				c_hash =   "".join(["".join([str(k) for k in i]) for i in c])
				# notInHashes = c_hash not in configs["hashes"]
				# configs["zeroes"][it_valid] = c*notInHashes
				# configs["hashes"].add(c_hash)
				# it_valid += notInHashes				 
				if c_hash not in configs["hashes"]:
					configs["hashes"].add(c_hash)
					configs["zeroes"][it_valid] = c
					it_valid +=1
		configs["latest_configs"][it]=np.array([[0]*3]*8,dtype=np.byte)
	configs["numberOfconfigs"] = it_valid
	configs["latest_configs"],configs["zeroes"] = configs["zeroes"],configs["latest_configs"]
...
def generate_all_configs(initial_config):
	...
	configs = { "latest_configs":[initial_config], "hashes": set(initial_hash) }
	configs["latest_configs"] = np.array([[[0]*3]*8]*int(1.5*10**6),dtype=np.byte)
	configs["latest_configs"][0] = initial_config
	configs["zeroes"] = np.array([[[0]*3]*8]*int(1.5*10**6),dtype=np.byte)
	configs["numberOfconfigs"] = 1
	print(np.ravel( configs["latest_configs"][0]).T)
	while any( [ i == 1 for i in np.ravel( configs["latest_configs"][0]).T]):
		generate_for_one_set_of_configs(configs)
		last_key = last_key +1
		print(last_key, (configs["numberOfconfigs"]),str(time.time()-init)+ " seconds")

# previous config in this case is a numpy matrix
def rotate(previous_config,direction,axis):
	new_config = np.array([[k for k in i ] for i in previous_config])
	...
	for index,col in enumerate(cubes):
		for axis in axes:
			...
			new_config[col1][axis[0]] = previous_config[col2][axis[1]]
	return new_config
...
generate_all_configs(initial_config)
